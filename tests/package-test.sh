...
# Mica OS, the copyright header names Mica OS, and no tracked file or path
# carries the retired project name (the pattern below spells it as m[o]s so this
...
RETIRED='(^|[^a-z0-9])m[o]s([^a-z0-9]|$)'
hits="$(git grep -nIiE "${RETIRED}" || true)$(git ls-files | grep -iE "${RETIRED}" || true)"
if [ -z "${hits}" ]; then pass "no tracked file or path carries the retired project name"; else fail "the retired project name remains:
${hits}"; fi
